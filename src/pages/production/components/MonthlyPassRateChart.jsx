...
export const MonthlyPassRateChart = ({ className }) => {
  ...
  const { data: rawData, isLoading } = useQuery({
    queryKey: ["manager-pass-rate-trend", filter],
    queryFn: () =>
  ...
  const latestValue =
    data && data.length > 0 ? data[data.length - 1].passRate : 0;
      ...
      <CardContent>
        {isLoading ? (
          ...
          <div className="flex flex-col h-full">
            <div className="text-2xl font-bold mb-4">{latestValue}%</div>
          ...
          <p className="text-sm text-muted-foreground py-8 text-center">
            No inspection data available for this period.
          </p>
        )}
      </CardContent>
    </Card>
